...
def process_resources_use_cdn_reverse(path: str = "content/posts") -> None:
    """本函数将处理所有markdown文件, 将cdn引用转化为相对路径引用
    ...
    for root, _, files in os.walk(path):
        ...
        for file_name in files:
            ...
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                content = f.readlines()
                for i, line in enumerate(content):
                    if cdn_prefix in line:
                        print(cdn_prefix + root.replace('\\', '/') + '/resources/')
                        content[i] = line.replace(cdn_prefix + root.replace('\\', '/') + '/resources/', '![image](resources/')
                        print(f'changing `{line}` to `{content[i]}`'.replace('\n', ''))
            with open(file_path, 'w', encoding='utf-8') as f:
                f.writelines(content)
    print(f'Done! time usage: {time.time() - now}')
...
if __name__ == '__main__':
    ...
    process_resources_use_cdn_reverse()
